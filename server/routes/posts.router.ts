...
postsRouter.post('/makepost', upload.array('images'), async (req: Request, res: Response) => {
    try {
        ...
        if (!isPostCreateValid(newPost)) {
            throw new Error('Mongoose did not return a new post after it was created');
        }
...
postsRouter.post('/update/:_id', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const updatedPostFromDB = await posts.findByIdAndUpdate(updatedPost._id, updatedPost, { new: true });
...
postsRouter.post('/delete/:_id', async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(204).send({ status: true, result: 'Successfully deleted your post' });
    } catch (err: any) {
        res.status(500).send(err.message);
    }
});
